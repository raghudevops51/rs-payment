...
CART_PORT = os.getenv('CART_PORT', 8000)
...
def pay(id):
    ...
    # check user exists
    try:
        req = requests.get('http://{user}:{port}/check/{id}'.format(user=USER, port=USER_PORT, id=id))
    ...
    if not anonymous_user:
        try:
            req = requests.post('http://{user}:{port}/order/{id}'.format(user=USER, port=USER_PORT, id=id),
    ...
    # delete cart
    try:
        req = requests.delete('http://{cart}:{port}/cart/{id}'.format(cart=CART, port=CART_PORT, id=id));
